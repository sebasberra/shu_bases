...
CREATE TABLE regimenes_juridicos (
	...
	regjurest VARCHAR(100) NOT NULL,
...
CREATE TABLE subnodos (
	...
	KEY idx_fk_subnodos_id_nodo (id_nodo),
	CONSTRAINT fk_subnodos_id_nodo 
		FOREIGN KEY (id_nodo) 
...
UPDATE
	subnodos
SET
	nom_subnodo = UCASE(nom_subnodo);
	
	
/* subnodos no definidos */
UPDATE
	subnodos
SET
	nom_subnodo = 'NO DEFINIDO (ROSARIO)'
WHERE
	numregion = 4
AND numsubregion = 1;
UPDATE
	subnodos
SET
	nom_subnodo = 'NO DEFINIDO (SANTA FE)'
WHERE
	numregion = 3
AND numsubregion = 1;
UPDATE
	subnodos
SET
	nom_subnodo = 'NO DEFINIDO (RECONQUISTA)'
WHERE
	numregion = 1
AND numsubregion = 1;
...
/* arregla &QUOT; en nombres de efectores */
UPDATE efectores SET nom_efector='CS DRA. GRISELDA MERATI BARRIO SUDOESTE' WHERE id_efector=89;
UPDATE efectores SET nom_efector='DR ANTONIO JOSÉ LUIS MARTINELLI' WHERE id_efector=555;
