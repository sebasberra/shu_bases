...
CREATE  TABLE IF NOT EXISTS `servicios` (
  ...
  `tipo_servicio` TINYINT NOT NULL COMMENT '1=final; 2=intermedio; 3=general' ,
...
CREATE  TABLE IF NOT EXISTS `efectores_servicios` (
  ...
  `intermedio_gral` TINYINT NOT NULL COMMENT 'solo se define para servicios de consultorio externo que tengan el campo medico igual a cero. 0=no definido; 1=intermedio; 2=gral' ,
...
CREATE  TABLE IF NOT EXISTS `salas` (
  `id_sala` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'concatenacion del id_efector con el id_nro_sala. los nuevos valores de id_nro_sala se obtienen con la funcion get_next_nro_sala' ,
  `id_efector` INT UNSIGNED NOT NULL COMMENT 'id del efector de donde pertenece la sala' ,
  `id_nro_sala` CHAR(2) NOT NULL COMMENT 'la funcion get_next_nro_sala se calcula como MAX(id_nro_sala)+1 por cada efector' ,
  ...
  UNIQUE INDEX `idx_unique_id_efector_id_nro_sala` (`id_efector` ASC, `id_nro_sala` ASC) ,
...
CREATE  TABLE IF NOT EXISTS `servicios_salas` (
  `id_servicio_sala` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'se genera como la concatenacion del id_sala con el id_nro_servicio_sala' ,
  `id_efector` INT UNSIGNED NOT NULL COMMENT 'id_efector de donde es el servicio y la sala' ,
  `id_nro_servicio_sala` CHAR(2) NOT NULL COMMENT 'la funcion get_next_nro_servicio_sala se calcula como MAX(id_nro_servicio_sala)+1 por cada sala del efector' ,
  ...
  UNIQUE INDEX `idx_unique_id_efector_id_sala_id_nro_servicio_sala` (`id_efector` ASC, `id_sala` ASC, `id_nro_servicio_sala` ASC) ,
  UNIQUE INDEX `idx_unique_id_efector_id_efector_servicio_id_sala` (`id_efector` ASC, `id_efector_servicio` ASC, `id_sala` ASC) ,
  UNIQUE INDEX `idx_unique_id_sala_id_nro_servicio_sala` (`id_sala` ASC, `id_nro_servicio_sala` ASC) ,
...
-- -----------------------------------------------------
-- function get_next_nro_sala
-- -----------------------------------------------------
DROP function IF EXISTS `get_next_nro_sala`;
DELIMITER $$
CREATE DEFINER=`root`@`localhost` FUNCTION `get_next_nro_sala`(id_efector INTEGER) RETURNS char(2) CHARSET latin1
...
-- -----------------------------------------------------
-- function get_next_nro_servicio_sala
-- -----------------------------------------------------
DROP function IF EXISTS `get_next_nro_servicio_sala`;
DELIMITER $$
CREATE DEFINER=`root`@`localhost` FUNCTION `get_next_nro_servicio_sala`(id_efector INTEGER, id_sala INTEGER) RETURNS char(2) CHARSET latin1
