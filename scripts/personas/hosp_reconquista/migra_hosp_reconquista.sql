DROP TABLE IF EXISTS archivos_reconquista;
DROP TABLE IF EXISTS barrios_reconquista;
...
CREATE TABLE archivos_reconquista (
...
LOAD DATA LOCAL INFILE 'archivos.txt' 
	INTO TABLE archivos_reconquista
...
DELETE FROM archivos_reconquista WHERE arc_nro_hi='91234';


/* modifica el tipo de documento al formato de personas */
UPDATE archivos_reconquista
	SET arc_tipo_d =
		(CASE arc_tipo_d
			WHEN '0' THEN 'DNR'
			WHEN '1' THEN 'DNI'
...
ALTER TABLE archivos_reconquista
...
/* crea la tabla de barrios del hospital reconquista */
CREATE TABLE barrios_reconquista (
	Bar_codigo		VARCHAR(255)	NOT NULL,
	Bar_nombre		VARCHAR(255)	NOT NULL,
PRIMARY KEY (Bar_codigo)
)ENGINE=MyISAM ;

LOAD DATA LOCAL INFILE 'barrios.txt' 
	INTO TABLE barrios_reconquista
	FIELDS	TERMINATED BY '\t'
			OPTIONALLY ENCLOSED BY '"'
	LINES	TERMINATED BY '\r\n';